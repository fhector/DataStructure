/* Two Stacks - Wacp for keeping two stacks within a single linear array
    so that neither stack overflows until all of the memory is used and an
    entire stack is never shifted to a different location within an array */
...
int push1(struct stack *ps, int x)
{
...
int push2(struct stack *ps, int x)
{
